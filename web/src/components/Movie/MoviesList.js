...
const MoviesList = ({
    ...
    const find = useCallback((query, by) => {
        MoviesDataService.find(query, by, currentPage)
            .then(response => {
                setMovies(response.data.movies);
            })
            .catch(e => {
                            ...
                            <Col key={movie._id}>
                                <Card className="moviesListCard">
                                        <Card.Img
                                            ...
                                            src={movie.poster + "100px/180"}
                                            onError={handleImageError}
                                            alt="Card image"
                                        />
                                        {user && (
                                            favorites.includes(movie._id) ?
                                            <BsStarFill className="star starFill"
                                                        onClick={() => deleteFavorites(movie._id)}
                                            />
                                            :
                                            <BsStar className="star starEmpty"
